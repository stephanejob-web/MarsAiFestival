        ...
        <div
            ...
            style={
                large
                    ...
                    : { width: 140, height: 105, flexShrink: 0 }
            }
        >
            {isCameraOn && cameraPub ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#0d1117] to-[#131a2e]">
                    {profilPicture ? (
                        ...
                        <div
                            className={`flex ${avatarSize} items-center justify-center rounded-full bg-gradient-to-br from-aurora to-lavande font-extrabold text-deep-sky ${large ? "text-[0.95rem]" : "text-[0.75rem]"}`}
                        >
                            {initials}
                        </div>
                    )}
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex flex-1 flex-col gap-2 overflow-y-auto p-2">
                {participants.length === 0 ? (
                    ...
                    participants.map((p) => <WebcamTile key={p.identity} participant={p} large />)
                )}
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                    <span className="font-mono text-[0.78rem] font-bold" style={{ color }}>
                        {count}
                    </span>
...
const VoteBtn = ({
    ...
    <button
        ...
        className={`flex flex-1 flex-col items-center gap-1.5 rounded-xl border py-3 transition-all duration-200 disabled:cursor-not-allowed ${
            isActive
                ? `${bg} ${border}`
                : `border-white/[0.08] bg-white/[0.03] hover:${bg} hover:${border}`
        }`}
...
const ScreeningView = ({ screening }: Props): React.JSX.Element => {
    ...
    useEffect(() => {
        ...
        fetch(`${API}/api/votes?filmId=${screening.filmId}`, {
            ...
            .then(
                (data: {
                    success: boolean;
                    data: Array<{ jury_id: number; decision: string }>;
                }) => {
    ...
    useEffect(() => {
        ...
        socket.on(
            "screening:seek",
            ({ currentTime, seekedAt }: { currentTime: number; seekedAt: number }) => {
    ...
    const submitVote = useCallback(
        async (decision: VoteDecision): Promise<void> => {
            const token = localStorage.getItem("jury_token");
    ...
    const VOTES = [
        {
            key: "valide" as VoteDecision,
            label: "Valide",
            icon: "✅",
            color: "#4effce",
            glow: "rgba(78,255,206,0.6)",
            bg: "bg-aurora/15",
            border: "border-aurora/50",
        },
        {
            key: "arevoir" as VoteDecision,
            label: "À revoir",
            icon: "🔄",
            color: "#f5e642",
        ...
        {
            key: "in_discussion" as VoteDecision,
            label: "Discussion",
            icon: "💬",
            color: "#c084fc",
            glow: "rgba(192,132,252,0.6)",
            bg: "bg-lavande/15",
            border: "border-lavande/50",
        },
    ] as const;
    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden bg-[#020408]">
            ...
            <div
                ...
                style={{
                    background:
                        "radial-gradient(ellipse 70% 50% at 40% 50%, rgba(78,255,206,0.04) 0%, rgba(192,132,252,0.03) 50%, transparent 70%)",
                }}
            />
            ...
            <div className="flex shrink-0 items-center gap-4 border-b border-white/[0.06] bg-black/40 px-6 py-3 backdrop-blur-sm">
                ...
                {screening.country && (
                    <span className="text-[0.75rem] text-mist/50">{screening.country}</span>
                )}
                <span className="font-mono text-[0.78rem] text-mist/40">
            ...
            <div className="flex min-h-0 flex-1">
                    ...
                    {liveKitToken ? (
                        ...
                        <div className="flex flex-1 flex-col">
                            ...
                            <div className="border-t border-white/[0.06] p-3">
                                ...
                                {webcamError && (
                                    <p className="mb-2 text-[0.68rem] text-coral">{webcamError}</p>
                                )}
                                <button
                                    type="button"
                ...
                <div className="flex flex-1 items-center justify-center bg-black p-4">
                    <div
                        ...
                        style={{
                            boxShadow:
                                "0 0 0 1px rgba(78,255,206,0.12), 0 0 50px rgba(78,255,206,0.07), 0 24px 60px rgba(0,0,0,0.8)",
                        }}
                    >
                        {screening.videoUrl ? (
                            <>
                            ...
                            <div className="flex aspect-video w-full items-center justify-center bg-[#0a0f1e]">
                                <p className="text-[0.9rem] text-white-soft/30">
                ...
                <div className="flex w-[290px] shrink-0 flex-col border-l border-white/[0.06] bg-[#06090f]">
                    <div className="border-b border-white/[0.06] px-4 py-3">
                        <div className="flex items-center justify-between">
                            <span className="font-display text-[0.82rem] font-extrabold text-white-soft">
                                Voting Poll
                            </span>
                            {totalVotes > 0 && (
                        ...
                        <p className="mt-0.5 truncate text-[0.7rem] text-mist/50">
                            {screening.title}
                        </p>
                    </div>
                    ...
                    <div className="p-3">
                        ...
                        {voteStatus === "ok" && (
                            <p className="mt-2.5 text-center text-[0.72rem] font-semibold text-aurora">
                                ✓ Vote enregistré
                            </p>
                        )}
                        {voteStatus === "error" && (
                            <p className="mt-2.5 text-center text-[0.72rem] text-coral">
                                Erreur — réessaie
                            </p>
                        )}
                        {myVote && voteStatus === "idle" && (
                            <p className="mt-2.5 text-center text-[0.68rem] text-mist/40">
                                Tu as voté · clique pour changer
                            </p>
                        )}
                    </div>
                </div>
