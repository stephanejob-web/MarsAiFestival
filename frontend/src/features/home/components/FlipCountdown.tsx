...
const FlipDigit = ({ value }: { value: string }): React.JSX.Element => {
    ...
    useEffect(() => {
        ...
        setPrev(curr);
        setFlip(true);
        timer.current = setTimeout(() => { setCurr(value); setFlip(false); }, DUR);
        ...
        <div className="fk-slot">
            ...
            {!flip ? (
                <div className="fk-card fk-back">
                    <div className="fk-h fk-h-t"><span>{curr}</span></div>
                    <div className="fk-h fk-h-b"><span>{curr}</span></div>
                </div>
            ) : (
                <>
                    ...
                    <div className="fk-card" style={{ zIndex: 1 }}>
                        <div className="fk-h fk-h-b"><span>{prev}</span></div>
                    </div>
                    {/* front-top: old digit top — falls */}
                    <div className="fk-card">
                        <div className="fk-h fk-h-t fk-ft"><span>{prev}</span></div>
                    </div>
                    {/* front-bottom: new digit bottom — rises */}
                    <div className="fk-card">
                        <div className="fk-h fk-h-b fk-fb"><span>{curr}</span></div>
                    </div>
                </>
            )}
        </div>
    ...
    <div className="flex flex-col items-center gap-3">
        ...
        >
            ...
            <div style={{
                ...
                height: 1,
                background: "linear-gradient(90deg, transparent, rgba(78,255,206,0.5), transparent)",
                borderRadius: 1,
            }} />
            {value.split("").map((ch, i) => <FlipDigit key={i} value={ch} />)}
        </div>
...
const Sep = (): React.JSX.Element => (
    <div className="flex flex-col items-center gap-[0.2em] self-center mb-6" style={{ fontSize: "inherit" }}>
        {[0, 1].map((i) => (
            <div key={i} style={{
                width: "0.1em",
                height: "0.1em",
...
// ─── FlipCountdown ────────────────────────────────────────────────────────────
interface Props { days: number; hours: number; minutes: number; seconds: number; label: string }
...
const FlipCountdown = ({ days, hours, minutes, seconds, label }: Props): React.JSX.Element => {
    ...
    return (
        <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-3" style={{ fontSize: "clamp(2.4rem, 5.2vw, 4rem)" }}>
                {units.map((u, i) => (
